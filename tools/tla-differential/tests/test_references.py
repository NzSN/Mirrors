...
class References(unittest.TestCase):
 ...
 def test_live_pinned_sany_named_unnamed_implicit_and_level(self):
  ...
  for ident in ('rej-instance-definition-only','rej-instance-unnamed','rej-instance-implicit-substitution','rej-substitution-constant-by-state'):
   ...
   with tempfile.TemporaryDirectory() as d:
    ...
    o=sany.observe(fixture=capture.adapter_fixture(f),materialized=m,config={'sany_jar':JAR,'bridge_classes':CLASSES,'version':'1.8.0'},limits=process.ProcessLimits(),artifact_dir=m.input_dir/'art')
  ...
  with tempfile.TemporaryDirectory() as d:
   p=Path(d); m=capture.materialize_fixture(fixture=f,directory=p/'in'); (m.input_dir/'art').mkdir(); o=sany.observe(fixture=capture.adapter_fixture(f),materialized=m,config={'sany_jar':JAR,'bridge_classes':CLASSES},limits=process.ProcessLimits(),artifact_dir=m.input_dir/'art')
 ...
 def test_live_sany_syntax_name_and_level_rejections(self):
  ...
  for ident in ('rej-open-comment','rej-arity-mismatch'):
   f=next(x for x in c.fixtures if x.id==ident)
   with tempfile.TemporaryDirectory() as d:
    p=Path(d); m=capture.materialize_fixture(fixture=f,directory=p/'in'); (m.input_dir/'art').mkdir(); o=sany.observe(fixture=capture.adapter_fixture(f),materialized=m,config={'sany_jar':JAR,'bridge_classes':CLASSES},limits=process.ProcessLimits(),artifact_dir=m.input_dir/'art'); self.assertEqual(o['outcome'],'rejected')
