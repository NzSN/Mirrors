...
theorem decodeRpcResult_encodeRpcResult :
  ...
  | checkInvariant invariantStatus sessionId trace =>
      ...
      | none =>
          ...
          rfl
      | some v =>
          obtain ⟨w, hw, hval⟩ := decodeValue_bounded v (hok v (by simp)).1
  ...
  | query operatorValue sessionId state trace =>
      ...
      | none =>
          ...
          | none =>
              ...
              rfl
          | some vt =>
              obtain ⟨tw, htw, htval⟩ := decodeValue_bounded vt (htr vt (by simp [htrv])).1
              ...
              rfl
      | some vo =>
          obtain ⟨ow, how, hovval⟩ := decodeValue_bounded vo (ho vo (by simp [hopv])).1
          cases htrv : trace with
          | none =>
